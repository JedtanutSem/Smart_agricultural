...
rospy.loginfo(serial.__version__)
ser = serial.Serial(
    port='/dev/ttyACM0',\
    baudrate=9600,\
        ...
        if not messageStarted:
            ...
            if dataBufferIndex == 4:
                ...
                if (dataBuffer[0] + dataBuffer[1] + dataBuffer[2] + dataBuffer[3] == sum ):
                    data0 = dataBuffer[0]
                    #data1 = dataBuffer[1]
                    #print(str(dataBuffer[0]) + ' and ' + str(dataBuffer[1])+ ' and ' + str(dataBuffer[2])+ ' and ' + str(dataBuffer[3]))
                    vel_x = dataBuffer[0]
                    ang_z = dataBuffer[1]
                    vel_x_state = dataBuffer[2]
                    ang_z_state = dataBuffer[3]
